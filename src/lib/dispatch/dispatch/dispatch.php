...
function partial($view, $locals = null) {
  ...
  if (($template_root = config('dispatch.templates')) == null)
    if (config('dispatch.debug')) {
      error(500, "config('dispatch.templates') is not set.");
    } else {
  ...
  $view = $template_root.DIRECTORY_SEPARATOR.$view.'.html.php';
...
/**
 * Set the current context or retrive the context
 *
 * @param the context to be set
 */
function context($value = null) {
  return is_null($value) ? scope('$ctx$') : scope('$ctx$', $value);
} 
...
function render($view, $locals = array(), $layout = null) {
  ...
  $template_root = config('dispatch.templates');
  $template_root = (!$template_root ? 'layout' : $template_root);
  ...
  ob_start();
  require $template_root.DIRECTORY_SEPARATOR.$view.'.html.php';
  ...
  if ($layout !== false) {
    ...
    $layout = $template_root.DIRECTORY_SEPARATOR.$layout.'.html.php';
...
function on($method, $path, $callback = null, $isapi = false) {
  ...
  if (is_callable($callback) || is_string($callback)) {
    ...
    foreach ($routes[$method] as $pattern => $info) {
      ...
      $isapi = $info['isapi'];
      if (! is_callable($callback)) {
        if (null == config('dispatch.views') || null == config('dispatch.api')) {
          throw new \RuntimeException('Configure error, error code: 2313');
        }
        
        if ($isapi) {
          $path = config('dispatch.api');
          $path = $path['path'];
          if (!$path) {
            throw new \RuntimeException('Configure error, cant find the path to apis');
          }
          $file = rtrim($path, '/') . '/' . $callback . '.php';
        } else {
          $path = config('dispatch.views');
          $path = $path['path'];
          $file = rtrim($path, '/') . '/' . $callback . '.php';
        }
